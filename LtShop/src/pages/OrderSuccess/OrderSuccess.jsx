              ...
              {state.orders?.map((order, index) => (
                <React.Fragment key={index}>
                  <WrapperItemOrder key={order?.name}>
                    ...
                    <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '200px' }}>
                      <span>
                  ...
                  {index !== state.orders.length - 1 && <br />} {/* Add line break if not the last item */}
                </React.Fragment>
              ))}
              ...
              <div>
                <div>
                  <span style={{ fontSize: '16px', color: 'red' }}>
                    Tổng tiền: {state.payment === 'paypal' ? `$${(state?.totalPriceMemo / 25000).toFixed(2)}` : convertPrice(state?.totalPriceMemo)}
                  </span>
                </div>
              </div>
            </WrapperContainer>
